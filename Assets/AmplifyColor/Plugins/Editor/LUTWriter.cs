    ...
    public class LUTWriter
    {
        ...
        public void SaveLUT(LUTResult lutResult)
        {
            ...
            if (string.IsNullOrEmpty(assetpath))
            {
                ...
                {
                    string path = EditorUtility.SaveFilePanelInProject("Save as", Path.GetFileName(_texturePath), "png", "Please enter a file name to save the texture to");
            ...
            Texture2D text = AssetDatabase.LoadAssetAtPath(_texturePath, typeof(Texture2D)) as Texture2D;
            if (text != null)
            {
