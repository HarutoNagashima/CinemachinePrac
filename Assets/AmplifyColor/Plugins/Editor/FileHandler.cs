...
namespace AmplifyColor
{
    public class FileHandler
    {
        ...
        public void SaveToFile()
        {
            ...
            {
                string path = EditorUtility.SaveFilePanel("Save reference file", _settings.FilePath ?? "", "reference", "png");
                if (!string.IsNullOrEmpty(path))
                {
                    ...
                    Texture2D texture = imageResult.GenerateTexture2D();
        ...
        {
            string path = EditorUtility.OpenFilePanel("Load graded file", _settings.FilePath ?? "", "png");
            if (!string.IsNullOrEmpty(path))
            {
                _settings.FilePath = path;
                ...
                {
                    byte[] data = File.ReadAllBytes(_settings.FilePath);

                    Texture2D screenshottexture = new Texture2D(16, 16, TextureFormat.ARGB32, false);
                    screenshottexture.LoadImage(data);

                    ImageResult imageResult = ImageResult.FromTexture(screenshottexture);
