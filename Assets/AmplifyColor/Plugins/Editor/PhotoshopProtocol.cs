...
namespace AmplifyColor
{
    public class PhotoshopProtocol
    {
        ...
        public string SendJSCommand(string command)
        {
            ...
            NetworkStream stream = _connection.Stream;
            ...
            try
            {
                ...
                byte[] encryptedBytes = _encryptDecrypt.encrypt(allBytes);
                ...
                if (inComStatus != 0)
                {
                    ...
                    if (inType == JavascriptType || inType == ErrorstringType)
                    {
                        ...
                        byte[] bytemessage = new byte[inLength + 1];
                        int rr = stream.Read(bytemessage, 0, inLength);

                        if (rr > 0)
                        {
                            UTF8Encoding encoder = new UTF8Encoding();
                            string message = encoder.GetString(bytemessage);
                ...
                else
                {
                    ...
                    {
                        UTF8Encoding encoder = new UTF8Encoding();
                        string message = encoder.GetString(decryptedBytes.ToArray());
                        return message.Trim(new[] { '\0' });
                    }
        ...
        public string SendImage(ImageResult imageData)
        {
            ...
            try
            {
                ...
                BinaryWriter w = new BinaryWriter(temp_stream);
                ...
                byte[] data = imageData.GenerateRGBData();
                ...
                byte[] encryptedMessage = _encryptDecrypt.encrypt(message);
